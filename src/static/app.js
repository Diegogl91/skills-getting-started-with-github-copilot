document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.map((participant) => `
          <li>
            ...
            <button class="remove-participant" type="button" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${escapeHtml(participant)}" title="Remove participant">&times;</button>
          </li>
        `).join("");
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    const activity = decodeURIComponent(activityCard.dataset.activity);
